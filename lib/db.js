...
export async function listImages(filters, order) {
    ...
    const images = await sql`
      ...
      ` : sql``}
      FROM image
      ${labels.length > 0 || restrictedLabels.length > 0 ? sql`
...
export async function getUser(id, provider, userData) {
    const data = { ...userData }
    delete data['id'] // drop dynamic session id
    const result = await sql`
...
export async function writeLog(id, action, visitor, meta=null) {
    if (visitor.id !== undefined && adminIds.includes(visitor.id))
        return
    await sql`
      INSERT INTO log
      VALUES (${id}, ${visitor}, ${action}, NOW(), ${meta})
    `
}
...
export async function getLogUsers(filters) {
    ...
    const users = await sql`
      SELECT
        ...
        jsonb_agg(DISTINCT jsonb_build_object('browser', meta->'browser', 'device', meta->'device', 'ua', meta->'ua')) AS meta
      FROM log
      ${filters.label ? sql`
...
export async function getUserLog(filters) {
  ...
  if (filters.user)
      where.push(sql`visitor->>'id' = ${filters.user}`)

  if (filters.ip)
    where.push(sql`visitor->>'ip' = ${filters.ip}`)
  ...
  const images = await sql`
    SELECT
      ...
      (array_agg(visitor))[1] AS visitor,
