...
export default class Image {
    ...
    async updateMetadata() {
        ...
        const stime = info['CreateDate'] || info['DateTimeDigitized'] || info['ModifyDate']
        if (stime !== undefined)
            data.stime = stime
    ...
    async getResized(width) {
        await this.ensurePath()
        const { data, info } = await sharp(this.path, { failOnError: false })
            .rotate()
            .resize({ width: width })
            .jpeg()
    ...
    async makeThumbnail(size='m', force=false) {
        ...
        await sharp(this.path, { failOnError: false })
            .rotate()
